import React from "react";

const VisitAustralia = () => {
  return (
    <div className="min-h-screen bg-gray-50 pt-1 px-4 sm:px-6 md:px-10 pb-6">
      <div className="max-w-4xl mx-auto bg-white rounded-xl shadow-lg overflow-hidden">

        {/* Header Gradient: Australia Flag */}
        <div className="bg-linear-to-r from-blue-700 via-white to-red-600 px-6 py-6">
          <h1 className="text-3xl sm:text-4xl font-bold text-center text-gray-900">
            Australia – Visitor Visa (Subclass 600)
          </h1>
        </div>

        <div className="p-6 sm:p-8">
          <div className="mb-6">
            <h2 className="text-xl font-semibold text-gray-700 mb-2">Visa Name</h2>
            <p className="text-gray-600">Visitor Visa – Subclass 600</p>
          </div>

          <div className="mb-6">
            <h2 className="text-xl font-semibold text-gray-700 mb-2">Validity</h2>
            <p className="text-gray-600">3 to 12 months (single or multiple entry)</p>
          </div>

          <div className="mb-6">
            <h2 className="text-xl font-semibold text-gray-700 mb-2">Embassy / VFS Fee</h2>
            <p className="text-gray-600">Application Fee:AUD 190 (~QAR 460) + Biometrics (~210 QAR)</p>
          </div>

          <div className="mb-6">
            <h2 className="text-xl font-semibold text-gray-700 mb-2">Family Info</h2>
            <p className="text-gray-600">
              Each family member must apply separately; family may submit applications together.
            </p>
          </div>

          <div className="mb-6">
            <h2 className="text-xl font-semibold text-gray-700 mb-2">Official Links (Qatar)</h2>
            <ul className="list-disc list-inside text-gray-600 space-y-1">
              <li>
                <a
                  href="https://immi.homeaffairs.gov.au/visas/getting-a-visa/visa-listing/visitor-600"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-green-700 hover:underline"
                >
                  Australia Visitor Visa – Subclass 600 Official Page
                </a>
              </li>
              <li>
                <a
                  href="https://visa.vfsglobal.com/qat/en/aus/"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-green-700 hover:underline"
                >
                  VFS Global – Australia Visa in Qatar
                </a>
              </li>
            </ul>
          </div>

          <div>
            <h2 className="text-xl font-semibold text-gray-700 mb-2">Required Documents</h2>
            <ul className="list-disc list-inside text-gray-600 space-y-1">
              <li>Passport (valid at least 6 months)</li>
              <li>Visa application form (online via ImmiAccount)</li>
              <li>Passport-size photos</li>
              <li>Qatar ID (QID) copy</li>
              <li>Bank statements (last 6 months) and salary slips (3-6 months)</li>
              <li>Flight & hotel bookings / travel itinerary</li>
              <li>Travel insurance (recommended)</li>
              <li>Employment letter or sponsor letter</li>
              <li>Proof of ties to home country</li>
            </ul>
          </div>
        </div>
      </div>
    </div>
  );
};

export default VisitAustralia;
